...
description: Security-focused code review using OWASP Top 10 and CWE patterns. Adversarial debate with Gemini 3.1 Pro's extended thinking for deep vulnerability detection.
---

# Gemini Security Review

## Purpose
Security-focused review identifying vulnerabilities aligned with OWASP Top 10 2021 and common CWE patterns. Gemini 3.1 Pro's extended thinking excels at finding subtle security issues.

## When to Use
When changes touch auth, crypto, SQL, user input, file uploads, or APIs. Complements `/gemini-impl-review`.
...
### 3. Init + Render + Start
Init: `node "$RUNNER" init --skill-name gemini-security-review --working-dir "$PWD"`
Render scope template first (`working-tree`/`branch`/`full`) with `BASE_BRANCH`. Then render template=`round1` with `WORKING_DIR`, `SCOPE`, `EFFORT`, `BASE_BRANCH`, `SCOPE_SPECIFIC_INSTRUCTIONS`.
